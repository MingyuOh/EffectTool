...
VS_OUTPUT VS(VS_INPUT input)
{
	...
	vOut.p = float4(Input.p, 1.0f);
	vOut.p = mul(input.p, g_matWorld);
	vOut.p = mul(input.p, g_matView);
	vOut.p = mul(input.p, g_matProj);
	vOut.n = normalize(mul(vIn.n, (float3x3)g_matWorldInverse));
	vOut.t = input.t * 10;
	vOut.c = input.c * g_vColor;
	return vOut;
}
...
{
	float4 vTexColor = g_txDiffuse.Sample(sample0, input.t);
