...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CTabbedPane* pTabbedPane = new CTabbedPane(TRUE); // TRUE를 넣어주면 해제할 필요 없다.
	pTabbedPane->Create(_T(""), this, CRect(0, 0, 250, 250), TRUE, -1, 
		WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | 
		WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI);

	m_wndEffectTool.CreateEx(NULL, _T("Particle"), this, CRect(0, 0, 100, 100),
		TRUE, 1001,
		WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS |
		WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI);

	m_wndEffectTool.EnableDocking(CBRS_ALIGN_ANY);
	DockPane(&m_wndEffectTool);

	pTabbedPane->AddTab(&m_wndEffectTool);
	pTabbedPane->EnableDocking(CBRS_ALIGN_ANY); // 어디다가든 붙힐 수 있다는 뜻이다.
