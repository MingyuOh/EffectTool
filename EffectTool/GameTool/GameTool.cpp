...
// GameTool.cpp : 응용 프로그램에 대한 클래스 동작을 정의합니다.
//
...
BOOL CGameToolApp::InitInstance()
{
	...
	CGameToolView* pView = (CGameToolView*)pFrm->GetActiveView();

	m_Main.m_hWnd = pView->m_hWnd;
	CRect rcClient;
	pView->GetClientRect(rcClient);
	m_Main.m_iWindowWidth = rcClient.Width();
	m_Main.m_iWindowHeight = rcClient.Height();
	m_Main.m_hInstance = AfxGetInstanceHandle();
	I_Input.m_hWnd = pView->m_hWnd;

	if (m_Main.GameInit() == FALSE)
	{
		return FALSE;
	}
